...
class LabelInput(tk.Frame):
    ...
    def __init__(self,
        ...
        if input_class in (ttk.Checkbutton, ttk.Button, ttk.Radiobutton):
            ...
            self.input.grid(row=0, column=0, sticky=(tk.N + tk.E + tk.S + tk.W), padx=5)

        else:
            self.label = ttk.Label(self, text=label, **label_args)
            self.label.grid(row=0, column=0, sticky=(tk.N + tk.E + tk.S + tk.W), padx=5)
            input_args["textvariable"] = input_var

            self.input = input_class(self, **input_args)
            self.input.grid(row=1, column=0, sticky=(tk.N + tk.E + tk.S + tk.W), padx=5)
